...
})({"alFGU":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "ca9da57f3aee2e91";
"use strict";
...
},{}],"eLJ1k":[function(require,module,exports) {
var _firebase = require("./firebase");
var _jobClassifier = require("./jobClassifier");
function createJobBoard(jobBoard) {
    ...
    if (companyLogo.src != "") titleDiv.appendChild(companyLogo);
...
function createTable(jobBoard, jobPostings) {
...
async function app(jobBoards, jobPostings) {
    ...
    document.getElementById("back-button")?.addEventListener("click", ()=>{
        ...
        scrollTop();
        while(tableSubDiv.children.length > 0)tableSubDiv.removeChild(tableSubDiv.children[0]);
    });
    for (const jobBoard of jobBoards){
        ...
        board.addEventListener("click", async ()=>{
            ...
            if (!title) title = createTitle();
            if (jobBoard.name === "Company Website") title.innerText = "Jobs from Company Websites";
            else if (jobBoard.name === "Unknown") title.innerText = "Jobs from unknown sources";
            else title.innerText = `Jobs from ${jobBoard.name}`;
...
(async ()=>{
    const jobBoards = await _firebase.getJobBoards();
    const indexedJobPostings = _jobClassifier.indexJobsBySource();
    app(jobBoards, indexedJobPostings);
})();

},{"./firebase":"43Hc9","./jobClassifier":"ki5Zx"}],"43Hc9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getJobBoards", ()=>getJobBoards
);
parcelHelpers.export(exports, "getJobPostings", ()=>getJobPostings
);
...
async function getJobBoards() {
    const res = await _database.get(_database.child(dbRef, "job_boards")).then((snapshot)=>{
        if (snapshot.exists()) return snapshot.val();
        else throw new Error("Job Boards not found");
    }).catch((error)=>{
...
async function getJobPostings() {
    const rawJobPostings = await _database.get(_database.child(dbRef, "jobs")).then((snapshot)=>{
        if (snapshot.exists()) return snapshot.val();
        else throw new Error("Job Boards not found");
    }).catch((error)=>{
        throw new Error("Unknown error occurred");
    });
    return rawJobPostings.map((post)=>{
        return {
...
parcelHelpers.export(exports, "_TEST_ACCESS_forceRestClient", ()=>forceRestClient1
);
...
parcelHelpers.export(exports, "get", ()=>get1
);
...
parcelHelpers.export(exports, "ref", ()=>ref2
);
...
const name9 = "@firebase/database";
const version1 = "0.12.4";
/**
 * @license
 * Copyright 2019 Google LLC
 *
 ...
 */ class DOMStorageWrapper {
     ...
     */ set(key6, value2) {
        if (value2 == null) this.domStorage_.removeItem(this.prefixedName_(key6));
        else this.domStorage_.setItem(this.prefixedName_(key6), _util.stringify(value2));
    }
    /**
     * @returns The value that was stored under this key, or null
     */ get(key1) {
        const storedVal = this.domStorage_.getItem(this.prefixedName_(key1));
 ...
 */ class MemoryStorage {
    ...
    set(key3, value1) {
        if (value1 == null) delete this.cache_[key3];
        else this.cache_[key3] = value1;
    }
    get(key4) {
        if (_util.contains(this.cache_, key4)) return this.cache_[key4];
        return null;
    }
    remove(key5) {
        delete this.cache_[key5];
    }
}
/**
 * @license
 ...
 */ const enableLogging$1 = function(logger_, persistent) {
    ...
    if (logger_ === true) {
        ...
        logger1 = logClient.log.bind(logClient);
        if (persistent) SessionStorage.set('logging_enabled', true);
    } else if (typeof logger_ === 'function') logger1 = logger_;
    else {
        logger1 = null;
...
const log = function(...varArgs) {
    if (firstLog_ === true) {
        firstLog_ = false;
        if (logger1 === null && SessionStorage.get('logging_enabled') === true) enableLogging$1(true);
    }
    if (logger1) {
 ...
 */ class AppCheckTokenProvider {
    constructor(appName_, appCheckProvider1){
        this.appName_ = appName_;
        this.appCheckProvider = appCheckProvider1;
        this.appCheck = appCheckProvider1 === null || appCheckProvider1 === void 0 ? void 0 : appCheckProvider1.getImmediate({
            optional: true
        });
        if (!this.appCheck) appCheckProvider1 === null || appCheckProvider1 === void 0 || appCheckProvider1.get().then((appCheck)=>this.appCheck = appCheck
        );
    }
    ...
    addTokenChangeListener(listener5) {
        var _a;
        (_a = this.appCheckProvider) === null || _a === void 0 || _a.get().then((appCheck)=>appCheck.addTokenListener(listener5)
        );
    }
 ...
 */ class FirebaseAuthTokenProvider {
    ...
    getToken(forceRefresh1) {
        if (!this.auth_) return new Promise((resolve, reject)=>{
            ...
            // becomes available before the timoeout below expires.
            setTimeout(()=>{
                if (this.auth_) this.getToken(forceRefresh1).then(resolve, reject);
                else resolve(null);
            }, 0);
        });
        return this.auth_.getToken(forceRefresh1).catch((error)=>{
    ...
    addTokenChangeListener(listener1) {
        // TODO: We might want to wrap the listener and call it with no args to
        // avoid a leaky abstraction, but that makes removing the listener harder.
        if (this.auth_) this.auth_.addAuthTokenListener(listener1);
        else this.authProvider_.get().then((auth)=>auth.addAuthTokenListener(listener1)
        );
    }
    removeTokenChangeListener(listener2) {
        this.authProvider_.get().then((auth)=>auth.removeAuthTokenListener(listener2)
        );
    }
    notifyForInvalidToken() {
...
/* AuthTokenProvider that supplies a constant token. Used by Admin SDK or mockUserToken with emulators. */ class EmulatorTokenProvider {
    ...
    getToken(forceRefresh2) {
        return Promise.resolve({
            accessToken: this.accessToken
        });
    }
    addTokenChangeListener(listener3) {
        // Invoke the listener immediately to match the behavior in Firebase Auth
        // (see packages/auth/src/auth.js#L1807)
        listener3(this.accessToken);
    }
    removeTokenChangeListener(listener4) {
    }
    notifyForInvalidToken() {
    }
}
 ...
 */ class RepoInfo {
     ...
     */ constructor(host2, secure1, namespace1, webSocketOnly1, nodeAdmin1 = false, persistenceKey = '', includeNamespaceInQueryParams = false){
        this.secure = secure1;
        this.namespace = namespace1;
        this.webSocketOnly = webSocketOnly1;
        this.nodeAdmin = nodeAdmin1;
        this.persistenceKey = persistenceKey;
        this.includeNamespaceInQueryParams = includeNamespaceInQueryParams;
        this._host = host2.toLowerCase();
        this._domain = this._host.substr(this._host.indexOf('.') + 1);
        this.internalHost = PersistentStorage.get('host:' + host2) || this._host;
    }
    isCacheableHost() {
 ...
 */ class PacketReceiver {
    ...
    closeAfter(responseNum, callback4) {
        this.closeAfterResponse = responseNum;
        this.onClose = callback4;
     ...
     */ handleResponse(requestNum, data1) {
        this.pendingResponses[requestNum] = data1;
 ...
 */ class BrowserPollConnection {
     ...
     */ constructor(connId, repoInfo5, applicationId, appCheckToken3, authToken2, transportSessionId, lastSessionId){
        this.connId = connId;
        this.repoInfo = repoInfo5;
        this.applicationId = applicationId;
        this.appCheckToken = appCheckToken3;
        ...
        this.stats_ = statsManagerGetCollection(repoInfo5);
        this.urlFn = (params)=>{
            // Always add the token if we have one.
            if (this.appCheckToken) params[APP_CHECK_TOKEN_PARAM] = this.appCheckToken;
            return repoInfoConnectionURL(repoInfo5, LONG_POLLING, params);
        };
    }
    /**
     ...
     */ send(data11) {
        const dataStr = _util.stringify(data11);
     ...
     */ addDisconnectPingFrame(id3, pw) {
        if (_util.isNodeSdk()) return;
        this.myDisconnFrame = document.createElement('iframe');
        const urlParams = {
        };
        urlParams[FIREBASE_LONGPOLL_DISCONN_FRAME_REQUEST_PARAM] = 't';
        urlParams[FIREBASE_LONGPOLL_ID_PARAM] = id3;
 ...
 *********************************************************************************************/ class FirebaseIFrameScriptHolder {
     ...
     */ addTag(url1, loadCB) {
        ...
        this.doNodeLongPoll(url1, loadCB);
        else setTimeout(()=>{
            try {
                ...
                newScript.onerror = ()=>{
                    log('Long-poll script failed to load: ' + url1);
 ...
 */ class WebSocketConnection {
     ...
     */ constructor(connId1, repoInfo1, applicationId1, appCheckToken1, authToken1, transportSessionId1, lastSessionId1){
        ...
        this.connURL = WebSocketConnection.connectionURL_(repoInfo1, transportSessionId1, lastSessionId1, appCheckToken1);
     ...
     */ static connectionURL_(repoInfo2, transportSessionId2, lastSessionId2, appCheckToken2) {
        const urlParams = {
        };
        urlParams[VERSION_PARAM] = PROTOCOL_VERSION;
        ...
        if (transportSessionId2) urlParams[TRANSPORT_SESSION_PARAM] = transportSessionId2;
        if (lastSessionId2) urlParams[LAST_SESSION_PARAM] = lastSessionId2;
        if (appCheckToken2) urlParams[APP_CHECK_TOKEN_PARAM] = appCheckToken2;
        return repoInfoConnectionURL(repoInfo2, WEBSOCKET, urlParams);
    }
    /**
     * @param onMessage - Callback when messages arrive
     * @param onDisconnect - Callback with connection lost.
     */ open(onMessage1, onDisconnect3) {
        this.onDisconnect = onDisconnect3;
        this.onMessage = onMessage1;
        ...
        try {
            ...
            const error = e1.message || e1.data;
     ...
     */ send(data5) {
        this.resetKeepAlive();
        const dataStr = _util.stringify(data5);
        this.bytesSent += dataStr.length;
        this.stats_.incrementCounter('bytes_sent', dataStr.length);
 ...
 */ class TransportManager {
    ...
    initTransports_(repoInfo4) {
        const isWebSocketsAvailable = WebSocketConnection && WebSocketConnection['isAvailable']();
        let isSkipPollConnection = isWebSocketsAvailable && !WebSocketConnection.previouslyFailed();
        if (repoInfo4.webSocketOnly) {
 ...
 */ class Connection {
     ...
     */ constructor(id2, repoInfo_, applicationId_, appCheckToken_, authToken_, onMessage_1, onReady_, onDisconnect_, onKill_, lastSessionId3){
        this.id = id2;
    ...
    connReceiver_(conn1) {
        return (message)=>{
            if (this.state_ !== 2 /* DISCONNECTED */ ) {
                if (conn1 === this.rx_) this.onPrimaryMessageReceived_(message);
                else if (conn1 === this.secondaryConn_) this.onSecondaryMessageReceived_(message);
    ...
    onPrimaryMessageReceived_(parsedData1) {
        // Must refer to parsedData properties in quotes, so closure doesn't touch them.
        const layer = requireKey('t', parsedData1);
        const data = requireKey('d', parsedData1);
        if (layer === 'c') this.onControl_(data);
        else if (layer === 'd') this.onDataMessage_(data);
    }
    onDataMessage_(message1) {
        this.onPrimaryResponse_();
        // We don't do anything with data messages, just kick them up a level
        this.onMessage_(message1);
    }
    onPrimaryResponse_() {
        if (!this.isHealthy_) {
    ...
    onControl_(controlData1) {
        const cmd = requireKey(MESSAGE_TYPE, controlData1);
        if (MESSAGE_DATA in controlData1) {
            const payload = controlData1[MESSAGE_DATA];
            ...
            else if (cmd === CONTROL_ERROR) error1('Server Error: ' + payload);
            else if (cmd === CONTROL_PONG) {
                this.log_('got pong on primary.');
                this.onPrimaryResponse_();
                this.sendPingOnPrimaryIfNecessary_();
            } else error1('Unknown control packet command: ' + cmd);
        }
    }
    /**
    ...
    startUpgrade_(conn2) {
        this.secondaryConn_ = new conn2(this.nextTransportId_(), this.repoInfo_, this.applicationId_, this.appCheckToken_, this.authToken_, this.sessionId);
        // For certain transports (WebSockets), we need to send and receive several messages back and forth before we
        // can consider the transport healthy.
        this.secondaryResponsesRequired_ = conn2['responsesRequiredToBeHealthy'] || 0;
    ...
    onReset_(host1) {
        this.log_('Reset packet received.  New host: ' + host1);
    ...
    onConnectionEstablished_(conn3, timestamp) {
    ...
    onConnectionShutdown_(reason1) {
        this.log_('Connection shutdown command received. Shutting down...');
        if (this.onKill_) {
            this.onKill_(reason1);
            this.onKill_ = null;
        }
 ...
 * @interface
 */ class ServerActions {
    put(pathString5, data7, onComplete14, hash5) {
    }
    merge(pathString1, data8, onComplete1, hash1) {
    }
    /**
     * Refreshes the auth token for the current connection.
     * @param token - The authentication token
     */ refreshAuthToken(token5) {
    }
    /**
     * Refreshes the app check token for the current connection.
     * @param token The app check token
     */ refreshAppCheckToken(token1) {
    }
    onDisconnectPut(pathString2, data9, onComplete2) {
    }
    onDisconnectMerge(pathString3, data10, onComplete3) {
    }
    onDisconnectCancel(pathString4, onComplete4) {
    }
    reportStats(stats) {
    }
}
 ...
 */ class EventEmitter {
     ...
     */ trigger(eventType6, ...varArgs1) {
        if (Array.isArray(this.listeners_[eventType6])) {
            // Clone the list, since callbacks could add/remove listeners.
            const listeners = [
                ...this.listeners_[eventType6]
            ];
            for(let i = 0; i < listeners.length; i++)listeners[i].callback.apply(listeners[i].context, varArgs1);
        }
    }
    on(eventType1, callback1, context) {
        this.validateEventType_(eventType1);
        this.listeners_[eventType1] = this.listeners_[eventType1] || [];
        this.listeners_[eventType1].push({
            callback: callback1,
            context
        });
        const eventData = this.getInitialEvent(eventType1);
        if (eventData) callback1.apply(context, eventData);
    }
    off(eventType2, callback2, context1) {
        this.validateEventType_(eventType2);
        const listeners = this.listeners_[eventType2] || [];
        for(let i = 0; i < listeners.length; i++)if (listeners[i].callback === callback2 && (!context1 || context1 === listeners[i].context)) {
            listeners.splice(i, 1);
            return;
        }
    }
    validateEventType_(eventType3) {
        _util.assert(this.allowedEvents_.find((et)=>{
            return et === eventType3;
        }), 'Unknown event: ' + eventType3);
    }
}
/**
 * @license
 ...
 */ class OnlineMonitor extends EventEmitter {
    ...
    getInitialEvent(eventType4) {
        _util.assert(eventType4 === 'online', 'Unknown event type: ' + eventType4);
        return [
            this.online_
        ];
    }
 ...
 */ class Path {
     ...
     * tokens array
     */ constructor(pathOrString, pieceNum1){
        if (pieceNum1 === void 0) {
 ...
 */ class ValidationPath {
     ...
     */ constructor(path7, errorPrefix_){
        this.errorPrefix_ = errorPrefix_;
        this.parts_ = pathSlice(path7, 0);
 ...
 */ class VisibilityMonitor extends EventEmitter {
    ...
    getInitialEvent(eventType5) {
        _util.assert(eventType5 === 'visible', 'Unknown event type: ' + eventType5);
        return [
            this.visible_
        ];
    }
 ...
 */ class PersistentConnection extends ServerActions {
    ...
    sendRequest(action12, body, onResponse) {
    ...
    get(query10) {
        ...
        const deferred = new _util.Deferred();
        const request = {
            p: query10._path.toString(),
            q: query10._queryObject
        };
    ...
    listen(query2, currentHashFn2, tag6, onComplete5) {
        this.initConnection_();
        ...
        _util.assert(query2._queryParams.isDefault() || !query2._queryParams.loadsAllData(), 'listen() called for non-default but complete query');
        _util.assert(!this.listens.get(pathString).has(queryId), 'listen() called twice for same path/queryId.');
        const listenSpec = {
            onComplete: onComplete5,
            hashFn: currentHashFn2,
            query: query2,
            tag: tag6
        };
    ...
    sendGet_(index3) {
        const get = this.outstandingGets_[index3];
        this.sendRequest('g', get.request, (message)=>{
            delete this.outstandingGets_[index3];
            this.outstandingGetCount_--;
            if (this.outstandingGetCount_ === 0) this.outstandingGets_ = [];
    ...
    static warnOnListenWarnings_(payload1, query3) {
        if (payload1 && typeof payload1 === 'object' && _util.contains(payload1, 'w')) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const warnings = _util.safeGet(payload1, 'w');
    ...
    refreshAppCheckToken(token3) {
        this.appCheckToken_ = token3;
    ...
    /**
     * @inheritDoc
     */ unlisten(query4, tag1) {
        const pathString = query4._path.toString();
        const queryId = query4._queryIdentifier;
        this.log_('Unlisten called for ' + pathString + ' ' + queryId);
        _util.assert(query4._queryParams.isDefault() || !query4._queryParams.loadsAllData(), 'unlisten() called for non-default but complete query');
        const listen = this.removeListen_(pathString, queryId);
        if (listen && this.connected_) this.sendUnlisten_(pathString, queryId, query4._queryObject, tag1);
    }
    sendUnlisten_(pathString16, queryId2, queryObj, tag2) {
        this.log_('Unlisten on ' + pathString16 + ' for ' + queryId2);
        const req = {
            p: pathString16
        };
        const action = 'n';
    ...
    onDisconnectPut(pathString6, data17, onComplete6) {
        this.initConnection_();
        if (this.connected_) this.sendOnDisconnect_('o', pathString6, data17, onComplete6);
        else this.onDisconnectRequestQueue_.push({
            pathString: pathString6,
            action: 'o',
            data: data17,
            onComplete: onComplete6
        });
    }
    onDisconnectMerge(pathString7, data12, onComplete7) {
        this.initConnection_();
        if (this.connected_) this.sendOnDisconnect_('om', pathString7, data12, onComplete7);
        else this.onDisconnectRequestQueue_.push({
            pathString: pathString7,
            action: 'om',
            data: data12,
            onComplete: onComplete7
        });
    }
    onDisconnectCancel(pathString8, onComplete8) {
        this.initConnection_();
        if (this.connected_) this.sendOnDisconnect_('oc', pathString8, null, onComplete8);
        else this.onDisconnectRequestQueue_.push({
            pathString: pathString8,
            action: 'oc',
            data: null,
            onComplete: onComplete8
        });
    }
    sendOnDisconnect_(action1, pathString9, data13, onComplete9) {
        const request = {
            p: pathString9,
            /*data*/ d: data13
        };
        this.log_('onDisconnect ' + action1, request);
        this.sendRequest(action1, request, (response)=>{
            if (onComplete9) setTimeout(()=>{
                onComplete9(response[/*status*/ 's'], response[/* data */ 'd']);
            }, Math.floor(0));
        });
    }
    put(pathString10, data14, onComplete10, hash2) {
        this.putInternal('p', pathString10, data14, onComplete10, hash2);
    }
    merge(pathString11, data15, onComplete11, hash3) {
        this.putInternal('m', pathString11, data15, onComplete11, hash3);
    }
    putInternal(action2, pathString12, data16, onComplete12, hash4) {
        this.initConnection_();
        const request = {
            /*path*/ p: pathString12,
            /*data*/ d: data16
        };
        if (hash4 !== undefined) request[/*hash*/ 'h'] = hash4;
        // TODO: Only keep track of the most recent put for a given path?
        this.outstandingPuts_.push({
            action: action2,
            request,
            onComplete: onComplete12
        });
        this.outstandingPutCount_++;
        const index = this.outstandingPuts_.length - 1;
        if (this.connected_) this.sendPut_(index);
        else this.log_('Buffering put: ' + pathString12);
    }
    sendPut_(index1) {
        const action = this.outstandingPuts_[index1].action;
        const request = this.outstandingPuts_[index1].request;
        const onComplete = this.outstandingPuts_[index1].onComplete;
        this.outstandingPuts_[index1].queued = this.connected_;
        this.sendRequest(action, request, (message)=>{
            this.log_(action + ' response', message);
            delete this.outstandingPuts_[index1];
            this.outstandingPutCount_--;
    ...
    onDataMessage_(message2) {
        if ('r' in message2) {
            // this is a response
            this.log_('from server: ' + _util.stringify(message2));
            const reqNum = message2['r'];
            const onResponse = this.requestCBHash_[reqNum];
            if (onResponse) {
                delete this.requestCBHash_[reqNum];
                onResponse(message2[/*body*/ 'b']);
            }
        } else if ('error' in message2) throw 'A server-side error has occurred: ' + message2['error'];
        else if ('a' in message2) // a and b are action and body, respectively
        this.onDataPush_(message2['a'], message2['b']);
    }
    onDataPush_(action3, body1) {
        this.log_('handleServerMessage', action3, body1);
        if (action3 === 'd') this.onDataUpdate_(body1[/*path*/ 'p'], body1[/*data*/ 'd'], /*isMerge*/ false, body1['t']);
        else if (action3 === 'm') this.onDataUpdate_(body1[/*path*/ 'p'], body1[/*data*/ 'd'], /*isMerge=*/ true, body1['t']);
        else if (action3 === 'c') this.onListenRevoked_(body1[/*path*/ 'p'], body1[/*query*/ 'q']);
        else if (action3 === 'ac') this.onAuthRevoked_(body1[/*status code*/ 's'], body1[/* explanation */ 'd']);
        else if (action3 === 'apc') this.onAppCheckRevoked_(body1[/*status code*/ 's'], body1[/* explanation */ 'd']);
        else if (action3 === 'sd') this.onSecurityDebugPacket_(body1);
        else error1('Unrecognized action received from server: ' + _util.stringify(action3) + '\nAre you using the latest client?');
    }
    onReady_(timestamp1, sessionId) {
        this.log_('connection ready');
        this.connected_ = true;
        this.lastConnectionEstablishedTime_ = new Date().getTime();
        this.handleTimestamp_(timestamp1);
    ...
    onVisible_(visible1) {
        ...
        if (visible1 && !this.visible_ && this.reconnectDelay_ === this.maxReconnectDelay_) {
            this.log_('Window became visible.  Reducing delay.');
            this.reconnectDelay_ = RECONNECT_MIN_DELAY;
            if (!this.realtime_) this.scheduleConnect_(0);
        }
        this.visible_ = visible1;
    }
    onOnline_(online) {
        if (online) {
    ...
    resume(reason2) {
        log('Resuming connection for reason: ' + reason2);
        delete this.interruptReasons_[reason2];
        if (_util.isEmpty(this.interruptReasons_)) {
            this.reconnectDelay_ = RECONNECT_MIN_DELAY;
            if (!this.realtime_) this.scheduleConnect_(0);
        }
    }
    handleTimestamp_(timestamp2) {
        const delta = timestamp2 - new Date().getTime();
    ...
    onListenRevoked_(pathString13, query5) {
        // Remove the listen and manufacture a "permission_denied" error for the failed listen.
        let queryId;
        if (!query5) queryId = 'default';
        else queryId = query5.map((q)=>ObjectToUniqueKey(q)
        ).join('$');
        const listen = this.removeListen_(pathString13, queryId);
        if (listen && listen.onComplete) listen.onComplete('permission_denied');
    }
    removeListen_(pathString14, queryId1) {
        const normalizedPathString = new Path(pathString14).toString(); // normalize path.
        let listen;
        if (this.listens.has(normalizedPathString)) {
            const map = this.listens.get(normalizedPathString);
            listen = map.get(queryId1);
            map.delete(queryId1);
    ...
    onAppCheckRevoked_(statusCode1, explanation1) {
        log('App check token revoked: ' + statusCode1 + '/' + explanation1);
        this.appCheckToken_ = null;
        this.forceTokenRefresh_ = true;
        // Note: We don't close the connection as the developer may not have
        // enforcement enabled. The backend closes connections with enforcements.
        if (statusCode1 === 'invalid_token' || statusCode1 === 'permission_denied') {
            // We'll wait a couple times before logging the warning / increasing the
            // retry period since oauth tokens will report as "invalid" if they're
            // just expired. Plus there may be transient issues that resolve themselves.
    ...
    onSecurityDebugPacket_(body2) {
        if (this.securityDebugCallback_) this.securityDebugCallback_(body2);
        else if ('msg' in body2) console.log('FIREBASE: ' + body2['msg'].replace('\n', '\nFIREBASE: '));
    }
    restoreState_() {
 ...
 * limitations under the License.
 */ class NamedNode {
    constructor(name3, node5){
        this.name = name3;
        this.node = node5;
    }
    static Wrap(name4, node1) {
        return new NamedNode(name4, node1);
    }
}
/**
 * @license
 ...
 */ class Index {
     ...
     */ indexedValueChanged(oldNode, newNode4) {
        const oldWrapped = new NamedNode(MIN_NAME, oldNode);
        const newWrapped = new NamedNode(MIN_NAME, newNode4);
...
class KeyIndex extends Index {
    ...
    static set __EMPTY_NODE(val1) {
        __EMPTY_NODE = val1;
    }
    compare(a4, b4) {
        return nameCompare(a4.name, b4.name);
    }
    isDefinedOn(node2) {
    ...
    indexedValueChanged(oldNode1, newNode1) {
    ...
    makePost(indexValue4, name5) {
        _util.assert(typeof indexValue4 === 'string', 'KeyIndex indexValue must always be a string.');
        // We just use empty node, but it'll never be compared, since our comparator only looks at name.
        return new NamedNode(indexValue4, __EMPTY_NODE);
    }
    /**
 ...
 */ class SortedMapIterator {
     ...
     */ constructor(node3, startKey, comparator, isReverse_, resultGenerator_ = null){
        this.isReverse_ = isReverse_;
        this.resultGenerator_ = resultGenerator_;
        this.nodeStack_ = [];
        let cmp = 1;
        while(!node3.isEmpty()){
            cmp = startKey ? comparator(node3.key, startKey) : 1;
            // flip the comparison if we're going in reverse
            if (isReverse_) cmp *= -1;
            if (cmp < 0) {
                // This node is less than our start key. ignore it
                if (this.isReverse_) node3 = node3.left;
                else node3 = node3.right;
            } else if (cmp === 0) {
                ...
                this.nodeStack_.push(node3);
                if (this.isReverse_) node3 = node3.right;
                else node3 = node3.left;
            }
        }
    }
    getNext() {
 ...
 */ class LLRBNode {
     ...
     */ constructor(key19, value8, color3, left3, right3){
        this.key = key19;
        this.value = value8;
        this.color = color3 != null ? color3 : LLRBNode.RED;
        this.left = left3 != null ? left3 : SortedMap.EMPTY_NODE;
        this.right = right3 != null ? right3 : SortedMap.EMPTY_NODE;
    }
    /**
     ...
     */ copy(key7, value3, color1, left1, right1) {
        return new LLRBNode(key7 != null ? key7 : this.key, value3 != null ? value3 : this.value, color1 != null ? color1 : this.color, left1 != null ? left1 : this.left, right1 != null ? right1 : this.right);
    }
    /**
     ...
     */ inorderTraversal(action4) {
        return this.left.inorderTraversal(action4) || !!action4(this.key, this.value) || this.right.inorderTraversal(action4);
    }
    /**
     ...
     */ reverseTraversal(action5) {
        return this.right.reverseTraversal(action5) || action5(this.key, this.value) || this.left.reverseTraversal(action5);
    }
    /**
     ...
     */ insert(key8, value4, comparator1) {
        let n = this;
        const cmp = comparator1(key8, n.key);
        if (cmp < 0) n = n.copy(null, null, null, n.left.insert(key8, value4, comparator1), null);
        else if (cmp === 0) n = n.copy(null, value4, null, null, null);
        else n = n.copy(null, null, null, null, n.right.insert(key8, value4, comparator1));
        return n.fixUp_();
    }
    /**
     ...
     */ remove(key9, comparator2) {
        let n, smallest;
        n = this;
        if (comparator2(key9, n.key) < 0) {
            if (!n.left.isEmpty() && !n.left.isRed_() && !n.left.left.isRed_()) n = n.moveRedLeft_();
            n = n.copy(null, null, null, n.left.remove(key9, comparator2), null);
        } else {
            ...
            n = n.copy(null, null, null, null, n.right.remove(key9, comparator2));
        }
        return n.fixUp_();
    }
    /**
 ...
 */ class LLRBEmptyNode {
     ...
     *
     * @returns The node copy.
     */ copy(key10, value5, color2, left2, right2) {
        return this;
    }
    /**
     ...
     */ insert(key11, value6, comparator3) {
        return new LLRBNode(key11, value6, null);
    }
    /**
 ...
 */ class SortedMap {
     ...
     */ insert(key13, value7) {
        return new SortedMap(this.comparator_, this.root_.insert(key13, value7, this.comparator_).copy(null, null, LLRBNode.BLACK, null, null));
    }
    /**
     ...
     */ remove(key14) {
        return new SortedMap(this.comparator_, this.root_.remove(key14, this.comparator_).copy(null, null, LLRBNode.BLACK, null, null));
    }
    /**
     ...
     */ get(key15) {
        ...
        let node = this.root_;
        while(!node.isEmpty()){
            cmp = this.comparator_(key15, node.key);
     ...
     */ getPredecessorKey(key16) {
        let cmp, node = this.root_, rightParent = null;
        while(!node.isEmpty()){
            cmp = this.comparator_(key16, node.key);
            if (cmp === 0) {
     ...
     * @returns The first truthy value returned by action, or the last falsey
     *   value returned by action
     */ inorderTraversal(action8) {
        return this.root_.inorderTraversal(action8);
    }
    /**
     ...
     */ reverseTraversal(action9) {
        return this.root_.reverseTraversal(action9);
    }
    /**
    ...
    getIteratorFrom(key17, resultGenerator1) {
        return new SortedMapIterator(this.root_, key17, this.comparator_, false, resultGenerator1);
    }
    getReverseIteratorFrom(key18, resultGenerator2) {
        return new SortedMapIterator(this.root_, key18, this.comparator_, true, resultGenerator2);
    }
    getReverseIterator(resultGenerator3) {
        return new SortedMapIterator(this.root_, null, this.comparator_, true, resultGenerator3);
    }
}
/**
 ...
 */ class LeafNode {
    ...
    static set __childrenNodeConstructor(val2) {
        __childrenNodeConstructor = val2;
    }
    ...
    /** @inheritDoc */ getImmediateChild(childName8) {
        // Hack to treat priority as a regular child
        if (childName8 === '.priority') return this.priorityNode_;
        else return LeafNode.__childrenNodeConstructor.EMPTY_NODE;
    }
    /** @inheritDoc */ getChild(path1) {
        if (pathIsEmpty(path1)) return this;
        else if (pathGetFront(path1) === '.priority') return this.priorityNode_;
        else return LeafNode.__childrenNodeConstructor.EMPTY_NODE;
    }
    hasChild() {
        return false;
    }
    /** @inheritDoc */ getPredecessorChildName(childName1, childNode1) {
        return null;
    }
    /** @inheritDoc */ updateImmediateChild(childName2, newChildNode4) {
        if (childName2 === '.priority') return this.updatePriority(newChildNode4);
        else if (newChildNode4.isEmpty() && childName2 !== '.priority') return this;
        else return LeafNode.__childrenNodeConstructor.EMPTY_NODE.updateImmediateChild(childName2, newChildNode4).updatePriority(this.priorityNode_);
    }
    /** @inheritDoc */ updateChild(path2, newChildNode1) {
        const front = pathGetFront(path2);
        if (front === null) return newChildNode1;
        else if (newChildNode1.isEmpty() && front !== '.priority') return this;
        else {
            _util.assert(front !== '.priority' || pathGetLength(path2) === 1, '.priority must be the last token in a path');
            return this.updateImmediateChild(front, LeafNode.__childrenNodeConstructor.EMPTY_NODE.updateChild(pathPopFront(path2), newChildNode1));
        }
    }
    /** @inheritDoc */ isEmpty() {
        return false;
    ...
    /** @inheritDoc */ forEachChild(index2, action10) {
        return false;
    }
    val(exportFormat) {
    ...
    equals(other1) {
        if (other1 === this) return true;
        else if (other1.isLeafNode()) {
            const otherLeaf = other1;
...
class PriorityIndex extends Index {
    compare(a1, b1) {
        const aPriority = a1.node.getPriority();
        const bPriority = b1.node.getPriority();
        const indexCmp = aPriority.compareTo(bPriority);
        if (indexCmp === 0) return nameCompare(a1.name, b1.name);
        else return indexCmp;
    }
    isDefinedOn(node4) {
        return !node4.getPriority().isEmpty();
    }
    indexedValueChanged(oldNode2, newNode2) {
        return !oldNode2.getPriority().equals(newNode2.getPriority());
    }
    minPost() {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return NamedNode.MIN;
    ...
    makePost(indexValue1, name6) {
        const priorityNode = nodeFromJSON$1(indexValue1);
...
class Base12Num {
    constructor(length1){
        ...
        ;
        this.count = logBase2(length1 + 1);
        this.current_ = this.count - 1;
        const mask = bitMask(this.count);
        this.bits_ = length1 + 1 & mask;
    }
    nextBitIsOne() {
...
class IndexMap {
    ...
    addIndex(indexDefinition1, existingChildren) {
        _util.assert(indexDefinition1 !== KEY_INDEX, "KeyIndex always exists and isn't meant to be added to the IndexMap.");
        const childList = [];
        let sawIndexedValue = false;
        const iter = existingChildren.getIterator(NamedNode.Wrap);
        let next = iter.getNext();
        while(next){
            sawIndexedValue = sawIndexedValue || indexDefinition1.isDefinedOn(next.node);
            childList.push(next);
            next = iter.getNext();
        }
        let newIndex;
        if (sawIndexedValue) newIndex = buildChildSet(childList, indexDefinition1.getCompare());
        else newIndex = fallbackObject;
        const indexName = indexDefinition1.toString();
        const newIndexSet = Object.assign({
        }, this.indexSet_);
        newIndexSet[indexName] = indexDefinition1;
     ...
     */ addToIndexes(namedNode2, existingChildren1) {
        const newIndexes = _util.map(this.indexes_, (indexedChildren, indexName)=>{
            ...
            if (indexedChildren === fallbackObject) {
                ...
                if (index.isDefinedOn(namedNode2.node)) {
                    // We need to build this index
                    const childList = [];
                    const iter = existingChildren1.getIterator(NamedNode.Wrap);
                    let next = iter.getNext();
                    while(next){
                        if (next.name !== namedNode2.name) childList.push(next);
                        next = iter.getNext();
                    }
                    childList.push(namedNode2);
                    return buildChildSet(childList, index.getCompare());
                } else // No change, this remains a fallback
                return fallbackObject;
            } else {
                const existingSnap = existingChildren1.get(namedNode2.name);
                let newChildren = indexedChildren;
                if (existingSnap) newChildren = newChildren.remove(new NamedNode(namedNode2.name, existingSnap));
                return newChildren.insert(namedNode2, namedNode2.node);
            }
        });
     ...
     */ removeFromIndexes(namedNode1, existingChildren2) {
        const newIndexes = _util.map(this.indexes_, (indexedChildren)=>{
            ...
            return indexedChildren;
                ...
                const existingSnap = existingChildren2.get(namedNode1.name);
                if (existingSnap) return indexedChildren.remove(new NamedNode(namedNode1.name, existingSnap));
 ...
 */ class ChildrenNode {
    ...
    /** @inheritDoc */ updatePriority(newPriorityNode1) {
        if (this.children_.isEmpty()) // Don't allow priorities on empty nodes
        return this;
        else return new ChildrenNode(this.children_, newPriorityNode1, this.indexMap_);
    }
    /** @inheritDoc */ getImmediateChild(childName3) {
        // Hack to treat priority as a regular child
        if (childName3 === '.priority') return this.getPriority();
        else {
            const child = this.children_.get(childName3);
            return child === null ? EMPTY_NODE : child;
        }
    }
    /** @inheritDoc */ getChild(path3) {
        const front = pathGetFront(path3);
        if (front === null) return this;
        return this.getImmediateChild(front).getChild(pathPopFront(path3));
    }
    /** @inheritDoc */ hasChild(childName4) {
        return this.children_.get(childName4) !== null;
    }
    /** @inheritDoc */ updateImmediateChild(childName5, newChildNode2) {
        _util.assert(newChildNode2, 'We should always be passing snapshot nodes');
        if (childName5 === '.priority') return this.updatePriority(newChildNode2);
        else {
            const namedNode = new NamedNode(childName5, newChildNode2);
            let newChildren, newIndexMap;
            if (newChildNode2.isEmpty()) {
                newChildren = this.children_.remove(childName5);
                newIndexMap = this.indexMap_.removeFromIndexes(namedNode, this.children_);
            } else {
                newChildren = this.children_.insert(childName5, newChildNode2);
    ...
    /** @inheritDoc */ updateChild(path4, newChildNode3) {
        const front = pathGetFront(path4);
        if (front === null) return newChildNode3;
        else {
            _util.assert(pathGetFront(path4) !== '.priority' || pathGetLength(path4) === 1, '.priority must be the last token in a path');
            const newImmediateChild = this.getImmediateChild(front).updateChild(pathPopFront(path4), newChildNode3);
    ...
    /** @inheritDoc */ val(exportFormat1) {
        ...
        if (!exportFormat1 && allIntegerKeys && maxKey < 2 * numKeys) {
            ...
            if (exportFormat1 && !this.getPriority().isEmpty()) obj['.priority'] = this.getPriority().val();
            return obj;
        }
    }
    /** @inheritDoc */ hash() {
        if (this.lazyHash_ === null) {
            ...
            this.lazyHash_ = toHash === '' ? '' : sha11(toHash);
        }
        return this.lazyHash_;
    }
    /** @inheritDoc */ getPredecessorChildName(childName6, childNode2, index6) {
        const idx = this.resolveIndex_(index6);
        if (idx) {
            const predecessor = idx.getPredecessorKey(new NamedNode(childName6, childNode2));
            return predecessor ? predecessor.name : null;
        } else return this.children_.getPredecessorKey(childName6);
    }
    getFirstChildName(indexDefinition2) {
        const idx = this.resolveIndex_(indexDefinition2);
        if (idx) {
            const minKey = idx.minKey();
            return minKey && minKey.name;
        } else return this.children_.minKey();
    }
    getFirstChild(indexDefinition3) {
        const minKey = this.getFirstChildName(indexDefinition3);
        if (minKey) return new NamedNode(minKey, this.children_.get(minKey));
        else return null;
    }
    /**
     * Given an index, return the key name of the largest value we have, according to that index
     */ getLastChildName(indexDefinition4) {
        const idx = this.resolveIndex_(indexDefinition4);
        if (idx) {
            const maxKey = idx.maxKey();
            return maxKey && maxKey.name;
        } else return this.children_.maxKey();
    }
    getLastChild(indexDefinition5) {
        const maxKey = this.getLastChildName(indexDefinition5);
        if (maxKey) return new NamedNode(maxKey, this.children_.get(maxKey));
        else return null;
    }
    forEachChild(index4, action11) {
        const idx = this.resolveIndex_(index4);
        if (idx) return idx.inorderTraversal((wrappedNode)=>{
            return action11(wrappedNode.name, wrappedNode.node);
        });
        else return this.children_.inorderTraversal(action11);
    }
    getIterator(indexDefinition6) {
        return this.getIteratorFrom(indexDefinition6.minPost(), indexDefinition6);
    }
    getIteratorFrom(startPost1, indexDefinition7) {
        const idx = this.resolveIndex_(indexDefinition7);
        if (idx) return idx.getIteratorFrom(startPost1, (key)=>key
        );
        else {
            const iterator = this.children_.getIteratorFrom(startPost1.name, NamedNode.Wrap);
            let next = iterator.peek();
            while(next != null && indexDefinition7.compare(next, startPost1) < 0){
                iterator.getNext();
                next = iterator.peek();
            }
            return iterator;
    ...
    getReverseIterator(indexDefinition8) {
        return this.getReverseIteratorFrom(indexDefinition8.maxPost(), indexDefinition8);
    }
    getReverseIteratorFrom(endPost, indexDefinition9) {
        const idx = this.resolveIndex_(indexDefinition9);
        if (idx) return idx.getReverseIteratorFrom(endPost, (key)=>{
            ...
            let next = iterator.peek();
            while(next != null && indexDefinition9.compare(next, endPost) > 0){
                iterator.getNext();
                next = iterator.peek();
            }
            return iterator;
    ...
    compareTo(other2) {
        if (this.isEmpty()) {
            if (other2.isEmpty()) return 0;
            else return -1;
        } else if (other2.isLeafNode() || other2.isEmpty()) return 1;
        else if (other2 === MAX_NODE) return -1;
        else // Must be another node with children.
        return 0;
    }
    withIndex(indexDefinition10) {
        if (indexDefinition10 === KEY_INDEX || this.indexMap_.hasIndex(indexDefinition10)) return this;
        else {
            const newIndexMap = this.indexMap_.addIndex(indexDefinition10, this.children_);
            return new ChildrenNode(this.children_, this.priorityNode_, newIndexMap);
        }
    }
    isIndexed(index5) {
        return index5 === KEY_INDEX || this.indexMap_.hasIndex(index5);
    }
    equals(other3) {
        if (other3 === this) return true;
        else if (other3.isLeafNode()) return false;
        else {
            const otherChildrenNode = other3;
     ...
     * instead.
     *
     */ resolveIndex_(indexDefinition11) {
        if (indexDefinition11 === KEY_INDEX) return null;
        else return this.indexMap_.get(indexDefinition11.toString());
    }
}
 ...
 */ class PathIndex extends Index {
    ...
    extractChild(snap6) {
        return snap6.getChild(this.indexPath_);
    }
    isDefinedOn(node8) {
        return !node8.getChild(this.indexPath_).isEmpty();
    }
    compare(a2, b2) {
        const aChild = this.extractChild(a2.node);
        const bChild = this.extractChild(b2.node);
        const indexCmp = aChild.compareTo(bChild);
        if (indexCmp === 0) return nameCompare(a2.name, b2.name);
        else return indexCmp;
    }
    makePost(indexValue2, name7) {
        const valueNode = nodeFromJSON(indexValue2);
 ...
 */ class ValueIndex extends Index {
    compare(a3, b3) {
        ...
        if (indexCmp === 0) return nameCompare(a3.name, b3.name);
        else return indexCmp;
    }
    isDefinedOn(node6) {
        return true;
    }
    indexedValueChanged(oldNode3, newNode3) {
        return !oldNode3.equals(newNode3);
    }
    minPost() {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return NamedNode.MIN;
    ...
    makePost(indexValue3, name8) {
        const valueNode = nodeFromJSON(indexValue3);
        return new NamedNode(name8, valueNode);
    }
    /**
     * @returns String representation for inclusion in a query spec
     */ toString() {
 ...
 */ class IndexedFilter {
    ...
    updateChild(snap1, key20, newChild3, affectedPath3, source7, optChangeAccumulator) {
        _util.assert(snap1.isIndexed(this.index_), 'A node must be indexed if only a child is updated');
        const oldChild = snap1.getImmediateChild(key20);
        // Check if anything actually changed.
        if (oldChild.getChild(affectedPath3).equals(newChild3.getChild(affectedPath3))) {
            // There's an edge case where a child can enter or leave the view because affectedPath was set to null.
            // In this case, affectedPath will appear null in both the old and new snapshots.  So we need
            // to avoid treating these cases as "nothing changed."
            if (oldChild.isEmpty() === newChild3.isEmpty()) // Nothing changed.
            // This assert should be valid, but it's expensive (can dominate perf testing) so don't actually do it.
            //assert(oldChild.equals(newChild), 'Old and new snapshots should be equal.');
            return snap1;
        }
        if (optChangeAccumulator != null) {
            if (newChild3.isEmpty()) {
                if (snap1.hasChild(key20)) optChangeAccumulator.trackChildChange(changeChildRemoved(key20, oldChild));
                else _util.assert(snap1.isLeafNode(), 'A child remove without an old child only makes sense on a leaf node');
            } else if (oldChild.isEmpty()) optChangeAccumulator.trackChildChange(changeChildAdded(key20, newChild3));
            else optChangeAccumulator.trackChildChange(changeChildChanged(key20, newChild3, oldChild));
        }
        if (snap1.isLeafNode() && newChild3.isEmpty()) return snap1;
        else // Make sure the node is indexed
        return snap1.updateImmediateChild(key20, newChild3).withIndex(this.index_);
    }
    updateFullNode(oldSnap, newSnap, optChangeAccumulator1) {
        if (optChangeAccumulator1 != null) {
            if (!oldSnap.isLeafNode()) oldSnap.forEachChild(PRIORITY_INDEX, (key, childNode)=>{
                if (!newSnap.hasChild(key)) optChangeAccumulator1.trackChildChange(changeChildRemoved(key, childNode));
            });
            if (!newSnap.isLeafNode()) newSnap.forEachChild(PRIORITY_INDEX, (key, childNode)=>{
                if (oldSnap.hasChild(key)) {
                    const oldChild = oldSnap.getImmediateChild(key);
                    if (!oldChild.equals(childNode)) optChangeAccumulator1.trackChildChange(changeChildChanged(key, childNode, oldChild));
                } else optChangeAccumulator1.trackChildChange(changeChildAdded(key, childNode));
            });
        }
        return newSnap.withIndex(this.index_);
    }
    updatePriority(oldSnap1, newPriority3) {
        if (oldSnap1.isEmpty()) return ChildrenNode.EMPTY_NODE;
        else return oldSnap1.updatePriority(newPriority3);
    }
    filtersNodes() {
        return false;
    }
 ...
 */ class RangedFilter {
    constructor(params4){
        this.indexedFilter_ = new IndexedFilter(params4.getIndex());
        this.index_ = params4.getIndex();
        this.startPost_ = RangedFilter.getStartPost_(params4);
        this.endPost_ = RangedFilter.getEndPost_(params4);
    }
    getStartPost() {
    ...
    matches(node7) {
        return this.index_.compare(this.getStartPost(), node7) <= 0 && this.index_.compare(node7, this.getEndPost()) <= 0;
    }
    updateChild(snap2, key21, newChild1, affectedPath1, source1, optChangeAccumulator2) {
        if (!this.matches(new NamedNode(key21, newChild1))) newChild1 = ChildrenNode.EMPTY_NODE;
        return this.indexedFilter_.updateChild(snap2, key21, newChild1, affectedPath1, source1, optChangeAccumulator2);
    }
    updateFullNode(oldSnap2, newSnap1, optChangeAccumulator3) {
        if (newSnap1.isLeafNode()) // Make sure we have a children node with the correct index, not a leaf node;
        newSnap1 = ChildrenNode.EMPTY_NODE;
        let filtered = newSnap1.withIndex(this.index_);
        // Don't support priorities on queries
        filtered = filtered.updatePriority(ChildrenNode.EMPTY_NODE);
        const self = this;
        newSnap1.forEachChild(PRIORITY_INDEX, (key, childNode)=>{
            if (!self.matches(new NamedNode(key, childNode))) filtered = filtered.updateImmediateChild(key, ChildrenNode.EMPTY_NODE);
        });
        return this.indexedFilter_.updateFullNode(oldSnap2, filtered, optChangeAccumulator3);
    }
    updatePriority(oldSnap3, newPriority1) {
        // Don't support priorities on queries
        return oldSnap3;
    }
    filtersNodes() {
    ...
    static getStartPost_(params1) {
        if (params1.hasStart()) {
            const startName = params1.getIndexStartName();
            return params1.getIndex().makePost(params1.getIndexStartValue(), startName);
        } else return params1.getIndex().minPost();
    }
    static getEndPost_(params2) {
        if (params2.hasEnd()) {
            const endName = params2.getIndexEndName();
            return params2.getIndex().makePost(params2.getIndexEndValue(), endName);
        } else return params2.getIndex().maxPost();
    }
}
/**
 * @license
 ...
 */ class LimitedFilter {
    constructor(params3){
        this.rangedFilter_ = new RangedFilter(params3);
        this.index_ = params3.getIndex();
        this.limit_ = params3.getLimit();
        this.reverse_ = !params3.isViewFromLeft();
    }
    updateChild(snap3, key22, newChild2, affectedPath2, source2, optChangeAccumulator4) {
        if (!this.rangedFilter_.matches(new NamedNode(key22, newChild2))) newChild2 = ChildrenNode.EMPTY_NODE;
        if (snap3.getImmediateChild(key22).equals(newChild2)) // No change
        return snap3;
        else if (snap3.numChildren() < this.limit_) return this.rangedFilter_.getIndexedFilter().updateChild(snap3, key22, newChild2, affectedPath2, source2, optChangeAccumulator4);
        else return this.fullLimitUpdateChild_(snap3, key22, newChild2, source2, optChangeAccumulator4);
    }
    updateFullNode(oldSnap4, newSnap2, optChangeAccumulator5) {
        let filtered;
        if (newSnap2.isLeafNode() || newSnap2.isEmpty()) // Make sure we have a children node with the correct index, not a leaf node;
        filtered = ChildrenNode.EMPTY_NODE.withIndex(this.index_);
        else if (this.limit_ * 2 < newSnap2.numChildren() && newSnap2.isIndexed(this.index_)) {
            // Easier to build up a snapshot, since what we're given has more than twice the elements we want
            filtered = ChildrenNode.EMPTY_NODE.withIndex(this.index_);
            // anchor to the startPost, endPost, or last element as appropriate
            let iterator;
            if (this.reverse_) iterator = newSnap2.getReverseIteratorFrom(this.rangedFilter_.getEndPost(), this.index_);
            else iterator = newSnap2.getIteratorFrom(this.rangedFilter_.getStartPost(), this.index_);
            let count = 0;
            ...
            filtered = newSnap2.withIndex(this.index_);
            // Don't support priorities on queries
            filtered = filtered.updatePriority(ChildrenNode.EMPTY_NODE);
            let startPost;
            let endPost;
        ...
        return this.rangedFilter_.getIndexedFilter().updateFullNode(oldSnap4, filtered, optChangeAccumulator5);
    }
    updatePriority(oldSnap5, newPriority2) {
        // Don't support priorities on queries
        return oldSnap5;
    }
    filtersNodes() {
    ...
    fullLimitUpdateChild_(snap4, childKey1, childSnap1, source3, changeAccumulator) {
        ...
        const oldEventCache = snap4;
        _util.assert(oldEventCache.numChildren() === this.limit_, '');
        const newChildNamedNode = new NamedNode(childKey1, childSnap1);
        const windowBoundary = this.reverse_ ? oldEventCache.getFirstChild(this.index_) : oldEventCache.getLastChild(this.index_);
        const inRange = this.rangedFilter_.matches(newChildNamedNode);
        if (oldEventCache.hasChild(childKey1)) {
            const oldChildSnap = oldEventCache.getImmediateChild(childKey1);
            let nextChild = source3.getChildAfterChild(this.index_, windowBoundary, this.reverse_);
            while(nextChild != null && (nextChild.name === childKey1 || oldEventCache.hasChild(nextChild.name)))// There is a weird edge case where a node is updated as part of a merge in the write tree, but hasn't
            // been applied to the limited filter yet. Ignore this next child which will be updated later in
            // the limited filter...
            nextChild = source3.getChildAfterChild(this.index_, nextChild, this.reverse_);
            const compareNext = nextChild == null ? 1 : cmp(nextChild, newChildNamedNode);
            const remainsInWindow = inRange && !childSnap1.isEmpty() && compareNext >= 0;
            if (remainsInWindow) {
                if (changeAccumulator != null) changeAccumulator.trackChildChange(changeChildChanged(childKey1, childSnap1, oldChildSnap));
                return oldEventCache.updateImmediateChild(childKey1, childSnap1);
            } else {
                if (changeAccumulator != null) changeAccumulator.trackChildChange(changeChildRemoved(childKey1, oldChildSnap));
                const newEventCache = oldEventCache.updateImmediateChild(childKey1, ChildrenNode.EMPTY_NODE);
        ...
        } else if (childSnap1.isEmpty()) // we're deleting a node, but it was not in the window, so ignore it
        return snap4;
        else if (inRange) {
            if (cmp(windowBoundary, newChildNamedNode) >= 0) {
                if (changeAccumulator != null) {
                    changeAccumulator.trackChildChange(changeChildRemoved(windowBoundary.name, windowBoundary.node));
                    changeAccumulator.trackChildChange(changeChildAdded(childKey1, childSnap1));
                }
                return oldEventCache.updateImmediateChild(childKey1, childSnap1).updateImmediateChild(windowBoundary.name, ChildrenNode.EMPTY_NODE);
            } else return snap4;
        } else return snap4;
    }
}
 ...
 */ class ReadonlyRestClient extends ServerActions {
    ...
    static getListenId_(query6, tag3) {
        if (tag3 !== undefined) return 'tag$' + tag3;
        else {
            _util.assert(query6._queryParams.isDefault(), "should have a tag if it's not a default query.");
            return query6._path.toString();
        }
    }
    /** @inheritDoc */ listen(query7, currentHashFn1, tag4, onComplete13) {
        const pathString = query7._path.toString();
        this.log_('Listen called for ' + pathString + ' ' + query7._queryIdentifier);
        // Mark this listener so we can tell if it's removed.
        const listenId = ReadonlyRestClient.getListenId_(query7, tag4);
        const thisListen = {
        };
        this.listens_[listenId] = thisListen;
        const queryStringParameters = queryParamsToRestQueryStringParameters(query7._queryParams);
        this.restRequest_(pathString + '.json', queryStringParameters, (error, result)=>{
            ...
            if (error === null) this.onDataUpdate_(pathString, data, /*isMerge=*/ false, tag4);
            if (_util.safeGet(this.listens_, listenId) === thisListen) {
                let status;
                if (!error) status = 'ok';
                else if (error === 401) status = 'permission_denied';
                else status = 'rest_error:' + error;
                onComplete13(status, null);
            }
        });
    }
    /** @inheritDoc */ unlisten(query8, tag5) {
        const listenId = ReadonlyRestClient.getListenId_(query8, tag5);
        delete this.listens_[listenId];
    }
    get(query9) {
        const queryStringParameters = queryParamsToRestQueryStringParameters(query9._queryParams);
        const pathString = query9._path.toString();
        const deferred = new _util.Deferred();
        this.restRequest_(pathString + '.json', queryStringParameters, (error, result)=>{
            let data = result;
            if (error === 404) {
    ...
    /** @inheritDoc */ refreshAuthToken(token4) {
    // no-op since we just always call getToken.
    }
    /**
     * Performs a REST request to the given path, with the provided query string parameters,
     * and any auth credentials we have.
     */ restRequest_(pathString15, queryStringParameters = {
    }, callback3) {
        ...
        return Promise.all([
            ...
            const url = (this.repoInfo_.secure ? 'https://' : 'http://') + this.repoInfo_.host + pathString15 + '?' + 'ns=' + this.repoInfo_.namespace + _util.querystring(queryStringParameters);
            this.log_('Sending REST request for ' + url);
            const xhr = new XMLHttpRequest();
            xhr.onreadystatechange = ()=>{
                if (callback3 && xhr.readyState === 4) {
 ...
 */ class SnapshotHolder {
    ...
    updateSnapshot(path6, newSnapshotNode) {
        this.rootNode_ = this.rootNode_.updateChild(path6, newSnapshotNode);
    }
}
/**
 * @license
 ...
 */ class AckUserWrite {
     ...
     */ constructor(/** @inheritDoc */ path14, /** @inheritDoc */ affectedTree1, /** @inheritDoc */ revert1){
        this.path = path14;
        this.affectedTree = affectedTree1;
        this.revert = revert1;
        /** @inheritDoc */ this.type = OperationType1.ACK_USER_WRITE;
        /** @inheritDoc */ this.source = newOperationSourceUser();
    }
    operationForChild(childName12) {
        if (!pathIsEmpty(this.path)) {
            _util.assert(pathGetFront(this.path) === childName12, 'operationForChild called for unrelated child.');
            ...
            const childTree = this.affectedTree.subtree(new Path(childName12));
 ...
 */ class ListenComplete {
    constructor(source4, path8){
        this.source = source4;
        this.path = path8;
        /** @inheritDoc */ this.type = OperationType1.LISTEN_COMPLETE;
    }
    operationForChild(childName9) {
 ...
 * limitations under the License.
 */ class Overwrite {
    constructor(source5, path9, snap5){
        this.source = source5;
        this.path = path9;
        this.snap = snap5;
        /** @inheritDoc */ this.type = OperationType1.OVERWRITE;
    }
    operationForChild(childName10) {
        if (pathIsEmpty(this.path)) return new Overwrite(this.source, newEmptyPath(), this.snap.getImmediateChild(childName10));
 ...
 * limitations under the License.
 */ class Merge {
    constructor(/** @inheritDoc */ source6, /** @inheritDoc */ path10, /** @inheritDoc */ children2){
        this.source = source6;
        this.path = path10;
        this.children = children2;
        /** @inheritDoc */ this.type = OperationType1.MERGE;
    }
    operationForChild(childName11) {
        if (pathIsEmpty(this.path)) {
            const childTree = this.children.subtree(new Path(childName11));
            ...
            _util.assert(pathGetFront(this.path) === childName11, "Can't get a merge for a child not on the path of the operation");
 ...
 */ class CacheNode {
    ...
    isCompleteForPath(path11) {
        if (pathIsEmpty(path11)) return this.isFullyInitialized() && !this.filtered_;
        const childKey = pathGetFront(path11);
        return this.isCompleteForChild(childKey);
    }
    isCompleteForChild(key23) {
        return this.isFullyInitialized() && !this.filtered_ || this.node_.hasChild(key23);
    }
    getNode() {
        return this.node_;
    }
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ function newViewCache1(eventCache, serverCache) {
    return {
        eventCache,
        serverCache
    };
...
function viewCacheUpdateEventSnap(viewCache, eventSnap, complete, filtered) {
    return newViewCache1(new CacheNode(eventSnap, complete, filtered), viewCache.serverCache);
}
function viewCacheUpdateServerSnap(viewCache, serverSnap, complete, filtered) {
    return newViewCache1(viewCache.eventCache, new CacheNode(serverSnap, complete, filtered));
}
 ...
 */ class ImmutableTree {
    constructor(value9, children1 = EmptyChildren()){
        this.value = value9;
     ...
     */ findRootMostMatchingPathAndValue(relativePath7, predicate1) {
        if (this.value != null && predicate1(this.value)) return {
            path: newEmptyPath(),
            value: this.value
        };
        else {
            if (pathIsEmpty(relativePath7)) return null;
            else {
                const front = pathGetFront(relativePath7);
                const child = this.children.get(front);
                if (child !== null) {
                    const childExistingPathAndValue = child.findRootMostMatchingPathAndValue(pathPopFront(relativePath7), predicate1);
     ...
     * value in the tree
     */ findRootMostValueAndPath(relativePath1) {
        return this.findRootMostMatchingPathAndValue(relativePath1, ()=>true
        );
    }
    /**
     * @returns The subtree at the given path
     */ subtree(relativePath2) {
        if (pathIsEmpty(relativePath2)) return this;
        else {
            const front = pathGetFront(relativePath2);
            const childTree = this.children.get(front);
            if (childTree !== null) return childTree.subtree(pathPopFront(relativePath2));
     ...
     */ set(relativePath3, toSet) {
        if (pathIsEmpty(relativePath3)) return new ImmutableTree(toSet, this.children);
        else {
            const front = pathGetFront(relativePath3);
            const child = this.children.get(front) || new ImmutableTree(null);
            const newChild = child.set(pathPopFront(relativePath3), toSet);
     ...
     * @returns Resulting tree.
     */ remove(relativePath4) {
        if (pathIsEmpty(relativePath4)) {
            if (this.children.isEmpty()) return new ImmutableTree(null);
            else return new ImmutableTree(null, this.children);
        } else {
            const front = pathGetFront(relativePath4);
            const child = this.children.get(front);
            if (child) {
                const newChild = child.remove(pathPopFront(relativePath4));
                let newChildren;
     ...
     */ get(relativePath5) {
        if (pathIsEmpty(relativePath5)) return this.value;
        else {
            const front = pathGetFront(relativePath5);
            const child = this.children.get(front);
            if (child) return child.get(pathPopFront(relativePath5));
            else return null;
        }
    }
    /**
     ...
     * @returns Resulting tree.
     */ setTree(relativePath6, newTree) {
        if (pathIsEmpty(relativePath6)) return newTree;
        else {
            const front = pathGetFront(relativePath6);
            const child = this.children.get(front) || new ImmutableTree(null);
            const newChild = child.setTree(pathPopFront(relativePath6), newTree);
            let newChildren;
            if (newChild.isEmpty()) newChildren = this.children.remove(front);
            else newChildren = this.children.insert(front, newChild);
            return new ImmutableTree(this.value, newChildren);
     ...
     */ fold_(pathSoFar2, fn1) {
        const accum = {
        };
        this.children.inorderTraversal((childKey, childTree)=>{
            accum[childKey] = childTree.fold_(pathChild(pathSoFar2, childKey), fn1);
        });
        return fn1(pathSoFar2, this.value, accum);
    }
    /**
     * Find the first matching value on the given path. Return the result of applying f to it.
     */ findOnPath(path12, f) {
        return this.findOnPath_(path12, newEmptyPath(), f);
    }
    findOnPath_(pathToFollow, pathSoFar1, f1) {
        const result = this.value ? f1(pathSoFar1, this.value) : false;
        if (result) return result;
            ...
            if (pathIsEmpty(pathToFollow)) return null;
                ...
                if (nextChild) return nextChild.findOnPath_(pathPopFront(pathToFollow), pathChild(pathSoFar1, front), f1);
                else return null;
            }
        }
    }
    foreachOnPath(path13, f2) {
        return this.foreachOnPath_(path13, newEmptyPath(), f2);
    }
    foreachOnPath_(pathToFollow1, currentRelativePath, f3) {
        if (pathIsEmpty(pathToFollow1)) return this;
        else {
            if (this.value) f3(currentRelativePath, this.value);
            const front = pathGetFront(pathToFollow1);
            const nextChild = this.children.get(front);
            if (nextChild) return nextChild.foreachOnPath_(pathPopFront(pathToFollow1), pathChild(currentRelativePath, front), f3);
            else return new ImmutableTree(null);
        }
    }
    /**
     ...
     */ foreach(f4) {
        this.foreach_(newEmptyPath(), f4);
    }
    foreach_(currentRelativePath1, f5) {
        this.children.inorderTraversal((childName, childTree)=>{
            childTree.foreach_(pathChild(currentRelativePath1, childName), f5);
        });
        if (this.value) f5(currentRelativePath1, this.value);
    }
    foreachChild(f6) {
        this.children.inorderTraversal((childName, childTree)=>{
            if (childTree.value) f6(childName, childTree.value);
        });
    }
}
/**
 ...
 */ class WriteTreeCompleteChildSource {
    ...
    getCompleteChild(childKey2) {
        const node = this.viewCache_.eventCache;
        if (node.isCompleteForChild(childKey2)) return node.getNode().getImmediateChild(childKey2);
        else {
            const serverNode = this.optCompleteServerCache_ != null ? new CacheNode(this.optCompleteServerCache_, true, false) : this.viewCache_.serverCache;
            return writeTreeRefCalcCompleteChild(this.writes_, childKey2, serverNode);
        }
    }
    getChildAfterChild(index7, child2, reverse1) {
        const completeServerData = this.optCompleteServerCache_ != null ? this.optCompleteServerCache_ : viewCacheGetCompleteServerSnap(this.viewCache_);
        const nodes = writeTreeRefCalcIndexedSlice(this.writes_, completeServerData, child2, 1, reverse1, index7);
...
function viewProcessorApplyOperation(viewProcessor, oldViewCache, operation, writesCache, completeCache) {
    ...
    if (operation.type === OperationType1.OVERWRITE) {
    ...
    } else if (operation.type === OperationType1.MERGE) {
    ...
    } else if (operation.type === OperationType1.ACK_USER_WRITE) {
    ...
    } else if (operation.type === OperationType1.LISTEN_COMPLETE) newViewCache = viewProcessorListenComplete(viewProcessor, oldViewCache, operation.path, writesCache, accumulator);
 ...
 */ class View {
    constructor(query_1, initialViewCache){
        ...
        this.viewCache_ = newViewCache1(newEventCache, newServerCache);
 ...
 */ function viewApplyOperation(view, operation, writesCache, completeServerCache) {
    if (operation.type === OperationType1.MERGE && operation.source.queryId !== null) {
 ...
 */ function syncPointGetView(syncPoint, query, writesCache, serverCache, serverCacheComplete) {
    ...
    if (!view) {
        ...
        const viewCache = newViewCache1(new CacheNode(eventCache, eventCacheComplete, false), new CacheNode(serverCache, serverCacheComplete, false));
 ...
 */ class SyncTree {
     ...
     */ constructor(listenProvider_){
         ...
         */ this.pendingWriteTree_ = newWriteTree1();
 ...
 * @param cancelError - If a cancelError is provided, appropriate cancel events will be returned.
 * @returns Cancel events, if cancelError was provided.
 */ function syncTreeRemoveEventRegistration(syncTree, query11, eventRegistration, cancelError) {
    // Find the syncPoint first. Then deal with whether or not it has matching listeners
    const path = query11._path;
    const maybeSyncPoint = syncTree.syncPointTree_.get(path);
    let cancelEvents = [];
    // A removal on a default query affects all queries at that location. A removal on an indexed query, even one without
    // other query constraints, does *not* affect all queries at that location. So this check must be for 'default', and
    // not loadsAllData().
    if (maybeSyncPoint && (query11._queryIdentifier === 'default' || syncPointViewExistsForQuery(maybeSyncPoint, query11))) {
        const removedAndEvents = syncPointRemoveEventRegistration(maybeSyncPoint, query11, eventRegistration, cancelError);
        ...
        if (!covered && removed.length > 0 && !cancelError) {
            ...
            if (removingDefault) {
                ...
                syncTree.listenProvider_.stopListening(syncTreeQueryForListening_(query11), defaultTag);
 ...
 */ function syncTreeSetupListener_(syncTree, query, view1) {
    const path = query._path;
    const tag = syncTreeTagForQuery_(syncTree, query);
 ...
 */ class ExistingValueProvider {
    ...
    getImmediateChild(childName14) {
        const child = this.node_.getImmediateChild(childName14);
...
class DeferredValueProvider {
    constructor(syncTree1, path15){
        this.syncTree_ = syncTree1;
        this.path_ = path15;
    }
    getImmediateChild(childName13) {
        const childPath = pathChild(this.path_, childName13);
 ...
 */ class Tree {
     ...
     */ constructor(name10 = '', parent1 = null, node9 = {
        children: {
        },
        childCount: 0
    }){
 ...
 */ function eventListRaise(eventList) {
    for(let i = 0; i < eventList.events.length; i++){
        ...
        if (eventData !== null) {
            ...
            if (logger1) log('event: ' + eventData.toString());
 ...
 */ class DataEvent {
     ...
     */ constructor(eventType10, eventRegistration, snapshot, prevName){
        this.eventType = eventType10;
...
class CancelEvent {
    constructor(eventRegistration1, error5, path19){
 ...
 */ class CallbackContext {
    constructor(snapshotCallback, cancelCallback1){
        this.snapshotCallback = snapshotCallback;
        this.cancelCallback = cancelCallback1;
    }
    onValue(expDataSnapshot, previousChildName1) {
        this.snapshotCallback.call(null, expDataSnapshot, previousChildName1);
    }
    onCancel(error2) {
    ...
    matches(other6) {
        return this.snapshotCallback === other6.snapshotCallback || this.snapshotCallback.userCallback !== undefined && this.snapshotCallback.userCallback === other6.snapshotCallback.userCallback && this.snapshotCallback.context === other6.snapshotCallback.context;
    }
}
/**
 * @license
 ...
 */ class OnDisconnect {
     ...
     */ set(value11) {
        ...
        validateFirebaseDataArg('OnDisconnect.set', value11, this._path, false);
        const deferred = new _util.Deferred();
        repoOnDisconnectSet(this._repo, this._path, value11, deferred.wrapCallback(()=>{
        }));
        return deferred.promise;
    }
    /**
     ...
     * @returns Resolves when synchronization to the Database is complete.
     */ setWithPriority(value10, priority1) {
        validateWritablePath('OnDisconnect.setWithPriority', this._path);
        validateFirebaseDataArg('OnDisconnect.setWithPriority', value10, this._path, false);
        validatePriority('OnDisconnect.setWithPriority', priority1, false);
        const deferred = new _util.Deferred();
        repoOnDisconnectSetWithPriority(this._repo, this._path, value10, priority1, deferred.wrapCallback(()=>{
        }));
        return deferred.promise;
    }
    /**
     ...
     */ update(values1) {
        ...
        validateFirebaseMergeDataArg('OnDisconnect.update', values1, this._path, false);
        const deferred = new _util.Deferred();
        repoOnDisconnectUpdate(this._repo, this._path, values1, deferred.wrapCallback(()=>{
        }));
        return deferred.promise;
    }
}
 ...
 */ class QueryImpl {
    ...
    isEqual(other7) {
        other7 = _util.getModularInstance(other7);
        if (!(other7 instanceof QueryImpl)) return false;
        const sameRepo = this._repo === other7._repo;
        const samePath = pathEquals(this._path, other7._path);
        const sameQueryIdentifier = this._queryIdentifier === other7._queryIdentifier;
 ...
 */ class ReferenceImpl extends QueryImpl {
    /** @hideconstructor */ constructor(repo1, path16){
        super(repo1, path16, new QueryParams(), false);
    }
    get parent() {
 ...
 */ class DataSnapshot {
     ...
     */ child(path17) {
        const childPath = new Path(path17);
        const childRef = child1(this.ref, path17);
     ...
     *  `false`.
     */ hasChild(path18) {
        const childPath = new Path(path18);
 ...
 */ function refFromURL(db, url) {
    ...
    return ref2(db, parsedURL.path.toString());
}
/**
 ...
 */ class ValueEventRegistration {
    constructor(callbackContext2){
        this.callbackContext = callbackContext2;
    }
    respondsTo(eventType7) {
        return eventType7 === 'value';
    }
    createEvent(change1, query13) {
        const index = query13._queryParams.getIndex();
        return new DataEvent('value', this, new DataSnapshot(change1.snapshotNode, new ReferenceImpl(query13._repo, query13._path), index));
    }
    ...
    createCancelEvent(error3, path21) {
        if (this.callbackContext.hasCancelCallback) return new CancelEvent(this, error3, path21);
        else return null;
    }
    matches(other8) {
        if (!(other8 instanceof ValueEventRegistration)) return false;
        else if (!other8.callbackContext || !this.callbackContext) // If no callback specified, we consider it to match any callback.
        return true;
        else return other8.callbackContext.matches(this.callbackContext);
    }
    hasAnyCallback() {
 ...
 */ class ChildEventRegistration {
    constructor(eventType8, callbackContext1){
        this.eventType = eventType8;
        this.callbackContext = callbackContext1;
    }
    respondsTo(eventType9) {
        let eventToCheck = eventType9 === 'children_added' ? 'child_added' : eventType9;
        eventToCheck = eventToCheck === 'children_removed' ? 'child_removed' : eventToCheck;
        return this.eventType === eventToCheck;
    }
    createCancelEvent(error4, path20) {
        if (this.callbackContext.hasCancelCallback) return new CancelEvent(this, error4, path20);
        else return null;
    }
    createEvent(change2, query12) {
        _util.assert(change2.childName != null, 'Child events should have a childName.');
        const childRef = child1(new ReferenceImpl(query12._repo, query12._path), change2.childName);
        const index = query12._queryParams.getIndex();
        return new DataEvent(change2.type, this, new DataSnapshot(change2.snapshotNode, childRef, index), change2.prevName);
    }
    getEventRunner(eventData1) {
        if (eventData1.getEventType() === 'cancel') return ()=>this.callbackContext.onCancel(eventData1.error)
        ;
        else return ()=>this.callbackContext.onValue(eventData1.snapshot, eventData1.prevName)
        ;
    }
    matches(other9) {
        if (other9 instanceof ChildEventRegistration) return this.eventType === other9.eventType && (!this.callbackContext || !other9.callbackContext || this.callbackContext.matches(other9.callbackContext));
        return false;
    }
    hasAnyCallback() {
...
class QueryEndAtConstraint extends QueryConstraint {
    ...
    _apply(query24) {
        validateFirebaseDataArg('endAt', this._value, query24._path, true);
        const newParams = queryParamsEndAt(query24._queryParams, this._value, this._key);
        validateLimit(newParams);
        validateQueryEndpoints(newParams);
        if (query24._queryParams.hasEnd()) throw new Error("endAt: Starting point was already set (by another call to endAt, endBefore or equalTo).");
        return new QueryImpl(query24._repo, query24._path, newParams, query24._orderByCalled);
    }
}
/**
...
class QueryEndBeforeConstraint extends QueryConstraint {
    ...
    _apply(query14) {
        ...
        const newParams = queryParamsEndBefore(query14._queryParams, this._value, this._key);
        validateLimit(newParams);
        validateQueryEndpoints(newParams);
        if (query14._queryParams.hasEnd()) throw new Error("endBefore: Starting point was already set (by another call to endAt, endBefore or equalTo).");
...
class QueryStartAtConstraint extends QueryConstraint {
    ...
    _apply(query15) {
        ...
        const newParams = queryParamsStartAt(query15._queryParams, this._value, this._key);
        validateLimit(newParams);
        validateQueryEndpoints(newParams);
        if (query15._queryParams.hasStart()) throw new Error("startAt: Starting point was already set (by another call to startAt, startBefore or equalTo).");
...
class QueryStartAfterConstraint extends QueryConstraint {
    ...
    _apply(query16) {
        validateFirebaseDataArg('startAfter', this._value, query16._path, false);
        const newParams = queryParamsStartAfter(query16._queryParams, this._value, this._key);
        validateLimit(newParams);
        validateQueryEndpoints(newParams);
        if (query16._queryParams.hasStart()) throw new Error("startAfter: Starting point was already set (by another call to startAt, startAfter, or equalTo).");
        return new QueryImpl(query16._repo, query16._path, newParams, query16._orderByCalled);
    }
}
/**
...
class QueryLimitToFirstConstraint extends QueryConstraint {
    ...
    _apply(query17) {
        if (query17._queryParams.hasLimit()) throw new Error("limitToFirst: Limit was already set (by another call to limitToFirst or limitToLast).");
        return new QueryImpl(query17._repo, query17._path, queryParamsLimitToFirst(query17._queryParams, this._limit), query17._orderByCalled);
    }
}
/**
...
class QueryLimitToLastConstraint extends QueryConstraint {
    ...
    _apply(query18) {
        if (query18._queryParams.hasLimit()) throw new Error("limitToLast: Limit was already set (by another call to limitToFirst or limitToLast).");
        return new QueryImpl(query18._repo, query18._path, queryParamsLimitToLast(query18._queryParams, this._limit), query18._orderByCalled);
    }
}
/**
...
class QueryOrderByChildConstraint extends QueryConstraint {
    ...
    _apply(query19) {
        validateNoPreviousOrderByCall(query19, 'orderByChild');
        const parsedPath = new Path(this._path);
        if (pathIsEmpty(parsedPath)) throw new Error('orderByChild: cannot pass in empty path. Use orderByValue() instead.');
        const index = new PathIndex(parsedPath);
        const newParams = queryParamsOrderBy(query19._queryParams, index);
        validateQueryEndpoints(newParams);
        return new QueryImpl(query19._repo, query19._path, newParams, /*orderByCalled=*/ true);
    }
}
/**
...
class QueryOrderByKeyConstraint extends QueryConstraint {
    _apply(query20) {
        validateNoPreviousOrderByCall(query20, 'orderByKey');
        const newParams = queryParamsOrderBy(query20._queryParams, KEY_INDEX);
        validateQueryEndpoints(newParams);
        return new QueryImpl(query20._repo, query20._path, newParams, /*orderByCalled=*/ true);
    }
}
/**
...
class QueryOrderByPriorityConstraint extends QueryConstraint {
    _apply(query21) {
        validateNoPreviousOrderByCall(query21, 'orderByPriority');
        const newParams = queryParamsOrderBy(query21._queryParams, PRIORITY_INDEX);
        validateQueryEndpoints(newParams);
        return new QueryImpl(query21._repo, query21._path, newParams, /*orderByCalled=*/ true);
    }
}
/**
...
class QueryOrderByValueConstraint extends QueryConstraint {
    _apply(query22) {
        validateNoPreviousOrderByCall(query22, 'orderByValue');
        const newParams = queryParamsOrderBy(query22._queryParams, VALUE_INDEX);
        validateQueryEndpoints(newParams);
        return new QueryImpl(query22._repo, query22._path, newParams, /*orderByCalled=*/ true);
    }
}
/**
...
class QueryEqualToValueConstraint extends QueryConstraint {
    ...
    _apply(query23) {
        validateFirebaseDataArg('equalTo', this._value, query23._path, false);
        if (query23._queryParams.hasStart()) throw new Error("equalTo: Starting point was already set (by another call to startAt/startAfter or equalTo).");
        if (query23._queryParams.hasEnd()) throw new Error("equalTo: Ending point was already set (by another call to endAt/endBefore or equalTo).");
        return new QueryEndAtConstraint(this._value, this._key)._apply(new QueryStartAtConstraint(this._value, this._key)._apply(query23));
    }
}
/**
 ...
 */ class Database {
    /** @hideconstructor */ constructor(_repoInternal, /** The {@link @firebase/app#FirebaseApp} associated with this Realtime Database instance. */ app1){
 ...
 */ function registerDatabase(variant) {
    ...
    _app.registerVersion(name9, version1, variant);
    // BUILD_TARGET will be replaced by values like esm5, esm2017, cjs5, etc during the compilation
    _app.registerVersion(name9, version1, 'esm2017');
}
/**
 * @license
 * Copyright 2020 Google LLC
 ...
 */ class TransactionResult {
    /** @hideconstructor */ constructor(/** Whether the transaction was successfully committed. */ committed1, /** The resulting data snapshot. */ snapshot1){
        this.committed = committed1;
 ...
 * @internal
 */ const forceRestClient1 = function(forceRestClient) {
...
},{"process":"lDnB8","@firebase/app":"lLbXy","@firebase/component":"1wISm","@firebase/util":"3yszE","@firebase/logger":"arr7M","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"lDnB8":[function(require,module,exports) {
...
},{}],"ki5Zx":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "jobSourceResolver", ()=>jobSourceResolver
);
parcelHelpers.export(exports, "indexJobsBySource", ()=>indexJobsBySource
);
var _firebase = require("./firebase");
async function parseJobBoards(jobBoards) {
    const parsedJobBoards = {
    };
...
async function classifyJobs(jobs, jobBoards) {
    return jobs.map((job)=>{
        const jobDomain = getDomain(job.jobURL);
        let source;
        if (!jobDomain) source = "Unknown";
        else if (jobBoards[jobDomain]) source = jobBoards[jobDomain].name;
        else if (jobDomain.toLowerCase().includes(job.companyName.toLowerCase())) source = "Company Website";
        else source = "Unknown";
        return {
            ...job,
            jobSource: source
...
function getDomain(jobURL) {
    if (jobURL) return jobURL.split("/")[2]?.split(".").slice(1).join(".");
}
async function jobSourceResolver() {
    const jobBoards = await parseJobBoards(await _firebase.getJobBoards());
    const jobPostings = await _firebase.getJobPostings();
    return await classifyJobs(jobPostings, jobBoards);
}
async function indexJobsBySource() {
    const jobSources = {
    };
    const jobs = await jobSourceResolver();
    jobs.forEach((job)=>{
        if (!jobSources[job.jobSource]) jobSources[job.jobSource] = [];
...
},{"./firebase":"43Hc9","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["alFGU","eLJ1k"], "eLJ1k", "parcelRequirebc44")

//# sourceMappingURL=index.3aee2e91.js.map
